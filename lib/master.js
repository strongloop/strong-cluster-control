...
function _recordSuicide(self, worker) {
  if (worker && !worker.suicide) {
    ...
    debug(
      'master - abormal exit by worker %s at time %s',
      worker.id, self._lastAbnormalExit);
  }
}

// Within a few seconds of an abnormal exit, throttle the worker creation rate
// to one per second. We wait for longer than the throttleDelay to see if the
// new worker is going to stay up, or just exit right away.
...
function startOne(callback) {
  ...
  function exit() {
    ...
    debug('master - one worker started exit',
      this.id, 'suicide?', this.suicide);
...
function shutdown(id) {
  ...
  worker.once('exit', function(code, signal) {
    debug('master - shutdown exit for worker %j', worker.id);
...
function start(options, callback) {
  ...
  debug('master - start %j cb %s', options, callback);
