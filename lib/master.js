...
function setSize(size, invisible) {
  ...
  if (!invisible) self.options.size = size;
...
function resize(worker) {
  ...
  if (worker) debug("_resize: worker %s", worker.id);
...
function restart() {
  ...
  function waitForStability(callback) {
    ...
    function check() {
      if (unstable()) return setTimeout(check, 1000).unref();
      return callback();
    }
    ...
    function unstable() {
      ...
      debug(
        "unstable? %j undersized? %j uptime %d ms",
        unstable,
        undersized,
        uptime
      );
...
function onExit(worker, code, signal) {
  ...
  debug(
    "on worker %s exit by %s suicide? %j",
    worker.id,
    reason,
    worker.suicide
  );
